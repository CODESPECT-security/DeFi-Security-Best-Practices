...
- [ ] All sensitive privileged operations (ownership transfers, upgrades, parameter changes) must go through a timelock in addition to multisig approval. The minimum timelock delay should scale with risk: 24 hours at launch, 48 hours above $1M TVL, and 72 hours or more above $10M TVL. Both queuing and executing a timelocked action must require the full multisig threshold defined above — the same TVL-based signer counts apply. Pausing is the single explicit exception — it must remain instantaneous to enable rapid response during an active exploit.
